...
Подразумивается, что результатмо работы CU является лишь набор управляющих сигналов. Далее результат работы CU - большое двоичное число, каждый бит которого - какой-то сигнал (описано ниже).  
...
instr: идентификатор инструкции
...
[type]:
    ...
    - 000100000000000 - if im_rdy
    - 001000000000000 - if dm_rdy
    - 010000000000000 - if io_rdy
    - 100000000000000 - if is_addr_dest


# Микрокод (БОЛЬ)

См control-unit-pseudocode.txt

Бранч по instr_type: alu, system, control, io

alu: 
  - выбрать операнд 1  
  - выбрать операнд 2 (если есть)
  - сделать операцию на alu, выставив instr как набор сигналов для alu
  - в зависимости от is_addr_dest отправить alu_res в dr и затем в data memory или отправить в регистра (в cu: reg)

system:
  - пока только hlt

control:
  - если jmp выгрузить абсолютный адрес из instruction memory в pc
  - если b** и условие выполнено, то pc + disp(из br 2 байта) -> pc
  - call/ret примерно описаны в isa + очев

io:
  - in/out примерно описаны в isa + очев
